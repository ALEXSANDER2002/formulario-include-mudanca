...
export function FormularioFeedback() {
  ...
  const calcularProgresso = () => {
    ...
    const camposObrigatorios = ["nome", "email", "curso", "semestre", "tipoContribuicao", "contribuicao"]
  ...
  return (
    ...
    >
      <CardContent className="p-0">
        ...
        <div className="bg-gradient-to-r from-[#0d1429] to-[#0a1225] p-4 md:p-6 border-b border-[#1a2547] relative">
          {/* Efeito de luz no canto */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500 rounded-full filter blur-[100px] opacity-10 -translate-y-1/2 translate-x-1/2"></div>

          {/* Título e subtítulo */}
          <div className="flex items-center gap-3 md:gap-4 relative z-10">
            <div className="bg-gradient-to-br from-blue-500 to-blue-700 rounded-lg p-2 md:p-2.5 shadow-[0_0_15px_rgba(0,102,255,0.3)] animate-pulse-slow">
              <MessageSquare className="h-5 w-5 md:h-6 md:h-6 text-white" />
            </div>
            <div>
              <h3 className="font-semibold text-lg md:text-xl text-white">Sua Opinião Importa</h3>
              <p className="text-sm md:text-base text-gray-400">Ajude-nos a construir um Centro Acadêmico melhor</p>
            </div>
          </div>
          ...
          <div className="mt-4 md:mt-6 relative z-10">
            <div className="flex justify-between text-xs md:text-sm mb-1.5 md:mb-2">
              <span className="text-gray-400 font-medium">Progresso do formulário</span>
              <span className="text-blue-400 font-semibold">{progressoFormulario}%</span>
            </div>
            <div className="h-1.5 md:h-2 w-full bg-[#1a2547] rounded-full overflow-hidden shadow-inner">
              <div
        ...
        <div className="p-4 md:p-8 relative">
          {/* Efeito de luz no canto */}
          <div className="absolute bottom-0 left-0 w-96 h-96 bg-blue-500 rounded-full filter blur-[120px] opacity-5 translate-y-1/2 -translate-x-1/2"></div>

          {/* Formulário */}
          <Form {...formulario}>
            <form onSubmit={formulario.handleSubmit(aoEnviar)} className="space-y-4 md:space-y-6 relative z-10">
              {/* Campo: Nome Completo */}
              <FormField
                control={formulario.control}
                name="nome"
                render={({ field }) => (
                  <FormItem className="animate-fade-in opacity-0" style={{ animationDelay: "0.1s" }}>
                    <FormLabel className="text-gray-300 font-medium">Nome Completo</FormLabel>
                    <FormControl>
                      <Input
                        placeholder="Seu nome completo"
                        {...field}
                        className="bg-[#0a1225] border-[#1a2547] text-white focus:border-blue-500 focus:ring-blue-500 rounded-lg h-10 md:h-11 shadow-sm transition-all duration-200"
                      />
                    </FormControl>
              ...
              <FormField
                ...
                name="email"
                render={({ field }) => (
                  <FormItem className="animate-fade-in opacity-0" style={{ animationDelay: "0.2s" }}>
                    <FormLabel className="text-gray-300 font-medium">Email Institucional</FormLabel>
                    <FormControl>
                      <Input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                <FormField
                  ...
                  name="curso"
                  render={({ field }) => (
                    <FormItem className="animate-fade-in opacity-0" style={{ animationDelay: "0.3s" }}>
                      <FormLabel className="text-gray-300 font-medium">Curso</FormLabel>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          <SelectTrigger className="bg-[#0a1225] border-[#1a2547] text-white focus:border-blue-500 focus:ring-blue-500 rounded-lg h-10 md:h-11 shadow-sm transition-all duration-200">
                            <SelectValue placeholder="Selecione" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent className="bg-[#0d1429] border-[#1a2547] text-white rounded-lg shadow-lg">
                          <SelectItem value="engcomp" className="focus:bg-blue-900/30 rounded-md">
                            Engenharia da Computação
                          </SelectItem>
                          <SelectItem value="engsoftware" className="focus:bg-blue-900/30 rounded-md">
                            Engenharia de Software
                          </SelectItem>
                          <SelectItem value="sistemas" className="focus:bg-blue-900/30 rounded-md">
                            Sistemas de Informação
                          </SelectItem>
                          <SelectItem value="cienciacomp" className="focus:bg-blue-900/30 rounded-md">
                            Ciência da Computação
                          </SelectItem>
                          <SelectItem value="redes" className="focus:bg-blue-900/30 rounded-md">
                            Redes de Computadores
                          </SelectItem>
                        </SelectContent>
                      </Select>
                      <FormMessage className="text-red-400 flex items-center gap-1 text-xs md:text-sm mt-1">
                        {formulario.formState.errors.curso && (
                          <AlertCircle className="h-3 w-3 md:h-3.5 md:w-3.5 flex-shrink-0" />
                        )}
                        {formulario.formState.errors.curso?.message}
                      </FormMessage>
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  name="semestre"
                  render={({ field }) => (
                    <FormItem className="animate-fade-in opacity-0" style={{ animationDelay: "0.4s" }}>
                      <FormLabel className="text-gray-300 font-medium">Semestre</FormLabel>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          <SelectTrigger className="bg-[#0a1225] border-[#1a2547] text-white focus:border-blue-500 focus:ring-blue-500 rounded-lg h-10 md:h-11 shadow-sm transition-all duration-200">
                            <SelectValue placeholder="Selecione" />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="animate-fade-in opacity-0" style={{ animationDelay: "0.5s" }}>
                    <FormLabel className="text-gray-300 font-medium">Tipo de Contribuição</FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger className="bg-[#0a1225] border-[#1a2547] text-white focus:border-blue-500 focus:ring-blue-500 rounded-lg h-10 md:h-11 shadow-sm transition-all duration-200">
                          <SelectValue placeholder="Selecione" />
              ...
              <FormField
                ...
                name="contribuicao"
                render={({ field }) => (
                  <FormItem className="animate-fade-in opacity-0" style={{ animationDelay: "0.6s" }}>
                    <FormLabel className="text-gray-300 font-medium">Sua Contribuição</FormLabel>
                    <FormControl>
                      <Textarea
                        placeholder="Compartilhe sua sugestão, reclamação ou ideia para o Centro Acadêmico..."
                        className="min-h-[120px] md:min-h-[150px] bg-[#0a1225] border-[#1a2547] text-white focus:border-blue-500 focus:ring-blue-500 rounded-lg shadow-sm transition-all duration-200 resize-y"
                        {...field}
                      />
                    </FormControl>
                    ...
                    <div className="flex justify-end mt-1 md:mt-2">
                      <span
                        className={`text-xs md:text-sm ${field.value.length > 900 ? "text-amber-400" : "text-gray-500"}`}
                      >
              ...
              <Button
                ...
                className={`w-full transition-all duration-500 rounded-lg h-11 md:h-12 shadow-lg animate-fade-in opacity-0 ${
                  progressoFormulario === 100
                    ? "bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-[0_0_15px_rgba(0,102,255,0.3)] transform hover:scale-[1.02]"
              ...
              >
                ...
                {estaEnviando ? (
                  ...
                  <>
                    <Send className="h-4 w-4 md:h-5 md:w-5 transition-transform duration-300 group-hover:translate-x-1" />
